...
summary(lm(abs(LOG_RR)~log(CONC_X)*PARAM_CLASS,water))
par(pin=c(3,3))
...
summary(lm(abs(LOG_RR)~log(CONC_X)*PARAM_CLASS,soil))
...
text(13,1.23,labels=expression(paste("Log RR"[STR]*" = e"^"0.11 * ln[Cu] - 0.18",","," p = 0.01")),cex=0.6)
text(13,1.16,labels=expression(paste("Log RR"[FUN]*" = e"^"0.04 * ln[Cu] + 0.01",","," p = 0.01")),cex=0.6)
```
...
summary(lm(log(PCHANGE)~log(CONC_X)+PARAM_CLASS,water))
par(pin=c(3,3))
...
text(850,10,labels="p = 0.31", cex=0.6)
...
summary(lm(log(PCHANGE)~log(CONC_X)+PARAM_CLASS,soil))
...
abline(lm(log10(PCHANGE)~log10(CONC_X),soil),col="darkgray",lwd=2,lty=3)

text(70,10,labels=expression(paste("Percent change (%) = e"^"0.29 * ln[Cu] - 2.77")),cex=0.6)
text(7500,10, labels="p < 0.001", cex=0.6)
```
