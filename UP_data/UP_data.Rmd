...
```{r Rsqr std error function & RDA bootstrap, include=F, eval=F}
  #Function to calculate standard error of Rsqr
...
r2.metafor <- matrix(NA, nrow=4, ncol=9, 
  ...
  r2.metafor[1:4,1] <- t(sapply(FUNmod, function(x) RsquareAdj(x)$r.squared))
  r2.metafor[1:4,2] <- t(sapply(FUNmod, function(x) SER2(x)$SE))
  r2.metafor[1:4,3] <- t(sapply(FUNdat, function(x) sd(boot(x,function(data,indices)
                        summary(lm(NPP~log(TOTMe),data[indices,]))$r.squared, R=10000)$t)))
  r2.metafor[1:4,4] <- t(sapply(STRmod, function(x) RsquareAdj(x)$r.squared))
  r2.metafor[1:4,5] <- t(sapply(STRmod, function(x) SER2.rda(x)$SE))
  r2.metafor[1:4,6] <- t(sapply(STRdat, function(x) sd(boot(x,function(data,indices)
                        RsquareAdj(rda(x[,9:51]~log(TOTMe),data[indices,]))$r.squared, R=10000)$t)))
  r2.metafor[1:4,7] <- r2.metafor[,1]-r2.metafor[,4]
  r2.metafor[1:4,8] <- sqrt(r2.metafor[,2]+r2.metafor[,5])
  r2.metafor[1:4,9] <- sqrt(r2.metafor[,3]+r2.metafor[,6])
