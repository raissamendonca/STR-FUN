...
```{r Rsqr std error function, tidy=TRUE}
...
SER2 <- function(model){
      ...
      R2.ci <- round(data.frame(R2-(abs(qt(0.05/2, n-2))*se),R2,R2+(abs(qt(0.05/2, n-2))*se),se),digits=3)
...
SER2(npp.hbst)
  ...
  #Bootstrapped R2 from regression with High-binding Salmon Trout data (npp.hbst)
library(boot)
boot.R2 <- boot(biof.HBST,function(data,indices)
