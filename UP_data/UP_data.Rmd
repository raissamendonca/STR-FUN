...
r2.metafor <- data.frame(matrix(NA, nrow=4, ncol=12, 
  ...
  r2.metafor[1:4,1] <- sapply(FUNmod, function(x) RsquareAdj(x)$r.squared) #FUN R2
  r2.metafor[1:4,2] <- sapply(FUNmod, function(x) SER2(x)$SE) #FUN SE formula
  r2.metafor[1:4,3] <- sapply(FUNdat, function(x) sd(boot(x,function(data,indices)
                        summary(lm(NPP~log(TOTMe),data[indices,],na.action=na.omit))$r.squared, R=10)$t)) #FUN SE bootstrap
  r2.metafor[1:4,4] <- sapply(STRmod, function(x) RsquareAdj(x)$r.squared) #STR R2
  r2.metafor[1:4,5] <- sapply(STRmod, function(x) SER2.rda(x)$SE) #STR SE formula
  r2.metafor[1:4,6] <- sapply(STRdat, function(x) sd(boot(x,function(data,indices)
                        RsquareAdj(rda(x[,9:51]~log(TOTMe),data[indices,]))$r.squared, R=10)$t)) #STR SE bootstrap
  r2.metafor[1:4,7] <- r2.metafor$FUN.R2-r2.metafor$STR.R2 #R2 diff as FUN-STR (i.e., if negative, FUN smaller than STR)
  r2.metafor[1:4,8] <- sqrt(r2.metafor$FUN.SE.form^2+r2.metafor$STR.SE.form^2) #Pooled SE formula as sqrt(SE1^2+SE2^2)
  r2.metafor[1:4,9] <- sqrt(r2.metafor$FUN.SE.boot^2+r2.metafor$STR.SE.boot^2) #Pooled SE bootstrap
  r2.metafor[1:4,10] <- as.character(c("Big Pup","Salmon Trout","Big Pup","Salmon Trout"))
  r2.metafor[1:4,11] <- as.character(c("Low-bind","Low-bind","High-bind","High-bind"))
  r2.metafor[1:4,12] <- as.character(c("LBBP","LBST","HBBP","HBST"))
...
base.boot <- rma.uni(r2.metafor$R2.diff,sei=r2.metafor$SE.boot.pooled,method="FE")
summary.rma(base.boot)
coef.summary.rma(summary.rma(base.boot))
weights(base.boot)
    ##yi can be written as formula (yi ~ mod1 + mod2 + mod3), which will override any 'mods'
```

```{r Test with mock dataset in metafor, echo=F, tidy=T}
  #Function to return all summaries for an rma model
rma.sum <- function(model) {
            sumList <- list(summary.rma(model),
                            coef.summary.rma(summary.rma(model)),
                            anova.rma(model))
            names(sumList) <- c("summary.rma","coef.summary","anova.rma")
            print(sumList)
}

mock.uni <- rma.uni(yi = R2dif~Fixed, sei = R2dif_SE, data = mock)
rma.sum(mock.uni)
summary.rma(mock.uni)
coef.summary.rma(summary.rma(mock.uni))
anova.rma(mock.uni)

mock.mv <- rma.mv(R2dif~Fixed, R2dif_SE, data=mock)
summary.rma(mock.mv)
```

```{r Comparison between book formula and rma() computed variables, echo=F, results=F}
book.wt <- matrix(NA, nrow=4, ncol=8, 
                  dimnames=list(c("Low-binding Big Pup","Low-binding Salmon Trout",
                                  "High binding Big Pup","High-binding Salmon Trout"),
                                c("BK Wt Effect Boot","BK Mean Wt Effect Boot",
                                  "RMA Wt Effect Boot","RMA Mean Wt Effect Boot",
                                  "BK Wt Effect Form","BK Mean Wt Effect Form",
                                  "RMA Wt Effect Form","RMA Mean Wt Effect Form")))

for(i in 1:nrow(r2.metafor)) {
  book.wt[[i,1]] <- r2.metafor[i,"R2.diff"]*(1/r2.metafor[i,"SE.boot.pooled"])
  book.wt[[i,2]] <- sum(r2.metafor[i,"R2.diff"]*(1/r2.metafor[i,"SE.boot.pooled"]))/sum(1/r2.metafor[i,"SE.boot.pooled"])
  book.wt[[i,3]] <- coef.summary.rma(summary.rma(base.boot))$estimate
  book.wt[[i,2]] <- sum(r2.metafor[i,"R2.diff"]*(1/r2.metafor[i,"SE.boot.pooled"]))/sum(1/r2.metafor[i,"SE.boot.pooled"])
}
